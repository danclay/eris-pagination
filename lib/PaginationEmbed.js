 ...
 * @property {String} [pageContent] Content for page # of #
 */
/**
...
class PaginationEmbed {
    ...
    constructor(message, pages = [], options = {}) {
        ...
        this.pageContent = options.pageContent || 'Page **{current}** of **{total}**'
    ...
    async initialize() {
        ...
        const messageContent = {
            content: (this.showPages) ? this.pageContent
            .replace('{current}', this.page)
            .replace('{total}', this.pages.length) : undefined,
		...
		if (this.options.useReply && !this.options.usedInteraction) {
			Object.assign(messageContent, {
				messageReferenceID: this.invoker.id,
					allowedMentions: {
					repliedUser: false
				}
			})
		...
		const authorID = this.invoker.author ? this.invoker.author.id : this.invoker.member.id;
        if (authorID === this.invoker._client.user.id) {
            this.message = await this.invoker.edit(messageContent);
        } else {
			if (this.options.usedInteraction) {
		...
		if (this.options.useButtons) {
			this.handler = new ReactionHandler.continuousActionRowStream(this.message, (userID) => userID === authorID, false, { maxMatches: this.maxMatches, time: this.timeout })
		} else {
			this.handler = new ReactionHandler.continuousReactionStream(this.message, (userID) => userID === authorID, false, { maxMatches: this.maxMatches, time: this.timeout });
		}
        ...
        if (!this.options.useButtons) {
			if (this.advanced) {
				...
				if (!this.disableDelete) await this.message.addReaction(this.delete);
			} else {
				await this.message.addReaction(this.back);
				await this.message.addReaction(this.forth);
			}
	...
	getComponents() {
		if (!this.options.useButtons) return [];
		const components = [];
			if (this.page !== 1) {
    ...
    run() {
        if (this.options.useButtons) {
			this.handler.on('action', async (event) => {
							...
							if (this.advanced) {
								return new Promise((resolve, reject) => {
									if (this.checkPerms()) {
										...
										reject('Insufficient permissions to remove reactions')
									}
								});
							}
						}
			...
			this.handler.on('reacted', async (event) => {
				switch (event.emoji.name) {
					case this.firstPage: {
						if (this.advanced) {
							if (this.checkPerms()) {
								await this.message.removeReaction(this.firstPage, this.invoker.author ? this.invoker.author.id : this.invoker.member.id);
							}
					...
					case this.back: {
						if (this.checkPerms()) {
							await this.message.removeReaction(this.back, this.invoker.author ? this.invoker.author.id : this.invoker.member.id);
						}
					...
					case this.forth: {
						if (this.checkPerms()) {
							await this.message.removeReaction(this.forth, this.invoker.author ? this.invoker.author.id : this.invoker.member.id);
						}
					...
					case this.lastPage: {
						if (this.advanced) {
							if (this.checkPerms()) {
								await this.message.removeReaction(this.lastPage, this.invoker.author ? this.invoker.author.id : this.invoker.member.id);
							}
